...
import com.stock.calculator.utils.CalculateUtil.retain
...
object WindowsUtil {
    ...
    var topBarHeight = 0
        get() {
            ...
            if (resourceId > 0) {
                field = resources.getDimensionPixelSize(resourceId)
            }
            return field
        }
        private set
    ...
    var statusBarHeight = 0
        get() {
            ...
            if (resourceId > 0) {
                field = AppConstant.getApp().getResources().getDimensionPixelSize(resourceId)
            }
            return field
        }
        private set
...
//        float densityDpi = metric.densityDpi;  // 屏幕密度DPI（120 / 160 / 240,小米4的是480）
        ...
        windowRatio = retain(ratio.toDouble(), 2)
        return windowRatio
    }
}